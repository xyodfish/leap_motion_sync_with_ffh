...
struct Vector {
     ...
     * @since 1.0
     */
    static const Vector& right() { return xAxis(); }
    /**
     ...
     * @since 1.0
     */
    static const Vector& up() { return yAxis(); }
    /**
     ...
     * @since 1.0
     */
    static const Vector& backward() { return zAxis(); }
     ...
     * @since 1.0
     */
    friend std::ostream& operator<<(std::ostream& out, const Vector& vector) { return out << vector.toString(); }
     ...
     * @since 1.0
     */
    float operator[](unsigned int index) const { return index < 3 ? (&x)[index] : 0.0f; }
     ...
     * @since 1.0
     */
    const float* toFloatPointer() const { return &x; /* Note: Assumes x, y, z are aligned in memory. */ }
...
struct FloatArray {
     ...
     * @since 1.0
     */
    float& operator[](unsigned int index) { return m_array[index]; }

    /**
     * Use the Float Array anywhere a float pointer can be used.
     * @since 1.0
     */
    operator float*() { return m_array; }

    /**
     * Use the Float Array anywhere a const float pointer can be used.
     * @since 1.0
     */
    operator const float*() const { return m_array; }
...
struct Matrix {
     ...
     * @since 1.0
     */
    Vector transformPoint(const Vector& in) const { return xBasis * in.x + yBasis * in.y + zBasis * in.z + origin; }
     ...
     * @since 1.0
     */
    Vector transformDirection(const Vector& in) const { return xBasis * in.x + yBasis * in.y + zBasis * in.z; }
    ...
    {
        return Matrix3x3Type(xBasis.x, xBasis.y, xBasis.z, yBasis.x, yBasis.y, yBasis.z, zBasis.x, zBasis.y, zBasis.z);
    }
    ...
    {
        std::stringstream result;
        result << "xBasis:" << xBasis.toString() << " yBasis:" << yBasis.toString() << " zBasis:" << zBasis.toString()
     ...
     * @since 1.0
     */
    friend std::ostream& operator<<(std::ostream& out, const Matrix& matrix) { return out << matrix.toString(); }
