...
namespace ar::Hardware::LeapMotion
{
    ...
    LpV2SyncFFH::LpV2SyncFFH() : ffhs_(handTotalNum)
    {
    }
    ...
    AR_RETURN_VALUE LpV2SyncFFH::parseConfig(const std::string &config)
    {
        ...
        for (auto i = 0; i < handTotalNum; ++i)
        {
            ...
            ffhs_[i]      = std::make_shared< FfhCtrl >(
                FfhUdpData{i, handNode["ip"].as< std::string >(), handNode["port"].as< int >()});

            lowerLimit_ = handNode["lower_limit"].as< std::vector< float > >();
            upperLimit_ = handNode["upper_limit"].as< std::vector< float > >();
        }
    ...
    AR_RETURN_VALUE LpV2SyncFFH::connectFFH()
    {
        for (auto i = 0; i < handTotalNum; ++i)
        {
            ...
            {
                spdlog::error(
                    "Cannot connect to {} device.", EnumUtils::intToString< int, FFHEnumClass >(ffhs_[i]->getId()));
                return AR_RETURN_VALUE::ACTION_FAIL;
            }
        }
    ...
    AR_RETURN_VALUE LpV2SyncFFH::disconnectFFH()
    {
        for (auto i = 0; i < handTotalNum; ++i)
        {
            ...
            {
                spdlog::error(
                    "Cannot disconnect {} device.", EnumUtils::intToString< int, FFHEnumClass >(ffhs_[i]->getId()));
                return AR_RETURN_VALUE::ACTION_FAIL;
            }
        }
    ...
    void LpV2SyncFFH::stop()
    {
        Runnable::stop();
        lmWrap.disconnect();
        // todo other stuff could be added here if needed
    }
    ...
    void LpV2SyncFFH::postProcessHandCmd()
    {
        for (auto i = 0; i < fingerNum_; ++i)
        {
            ...
            {
                if (handCmd.finger[i].angle[j] < lowerLimit_[j])
                {
                    handCmd.finger[i].angle[j] = lowerLimit_[j];
                }

                if (handCmd.finger[i].angle[j] > upperLimit_[j])
                {
                    handCmd.finger[i].angle[j] = upperLimit_[j];
                }
            }
        }
    ...
    void LpV2SyncFFH::executeLoop()
    {
        ...
        {
            runIteration();
            std::this_thread::sleep_for(20ms); // wait move
        }
    }
    ...
    bool LpV2SyncFFH::calPubFingerAngle(const LEAP_HAND &hand)
    {
        ...
        fingerCmd[0].angle[static_cast< int >(FJIndex::Middle)] =
            fingerCmd[0].angle[static_cast< int >(FJIndex::Distal)];
